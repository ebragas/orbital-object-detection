...
def draw_bounding_boxes(image, predictions, threshold, grouping_distance=50):
    '''TODO: document'''
    ...
    for coord, pred in predictions.items():
        if pred['probabilities'][1] > threshold:
            filtered_coords.append(make_tuple(coord))

    grouped_coords = non_max_suppression_fast(np.array(filtered_coords), overlapThresh=0)

    for coord in grouped_coords:
        draw.rectangle(tuple(coord), outline='red', width=3)
